...
ranks, likelihoods, outlooks, valuation/capital profiles, market price, data coverage, and SEC filing activity. Phase 12 adds a separate AI subsector leadership and competitive-landscape layer that summarizes peer-group strength, breadth, growth, cash flow, concentration, and company positioning without changing the original ranking score. Phase 13 adds an optional Learning Mode that explains the major figures, comparisons, and limitations in beginner-friendly language without changing any data or rankings.
"""
...
<div class="container">
  <div class="topbar">
    ...
    <div class="top-actions"><button class="theme-btn" id="theme-toggle">◐ Dark Mode</button><button class="learning-btn" id="learning-mode-toggle" aria-pressed="false">🎓 Learning Mode: Off</button></div>
  </div>

  <div class="eyebrow">Stock Digest Research</div>
  <div class="title-row"><h1>AI Market Intelligence</h1><span class="premium">PHASE 13 PREMIUM PREVIEW</span></div>
  <p class="timestamp" id="updated-at">Loading the latest AI company dataset...</p>
  <div class="notice"><strong>Research framework—not investment advice.</strong> The company score organizes reported financial facts and market momentum using the published methodology below. Phase 13 retains the weekly record, historical backtest, likelihood research, operating scenarios, capital-efficiency, relative-valuation, SEC filing-reaction, integrated-evidence, daily-change, and subsector layers, then adds an optional Learning Mode that explains how to read the research in beginner-friendly language. The original ranking score remains unchanged so the existing track record stays comparable. Every score remains research rather than advice. SEC figures can differ across issuers because companies use different permitted XBRL tags and fiscal calendars.</div>
  <div class="learning-mode-only learning-box">
    <h3>🎓 Learning Mode: How to use this page</h3>
    <p>This page brings together financial statements, stock-price behavior, company comparisons, and historical tests. Learning Mode does not change any score or calculation—it only adds explanations.</p>
    <ul>
      <li><span class="learning-term">Reported facts</span> come primarily from SEC filings.</li>
      <li><span class="learning-term">Research scores</span> organize several measurements into a 0–100 comparison; they are not school grades or guarantees.</li>
      <li><span class="learning-term">Peer comparisons</span> compare a company with the other companies in its current Stock Digest AI subsector.</li>
      <li><span class="learning-term">Likelihood estimates and scenarios</span> are model-generated research—not promises, analyst targets, or recommendations.</li>
      <li>Click a company in any linked table to open its detailed history, evidence, risks, and explanations.</li>
    </ul>
  </div>
  ...
  <div class="learning-mode-only learning-box"><h4>How to read the Weekly Leaders</h4><p><span class="learning-term">Official rank</span> is the rank saved at the beginning of the week. <span class="learning-term">Live rank</span> is today’s rank. Return shows the stock move since selection, while peer return shows the average move of comparable companies. Relative return is the difference between the two.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>What is a track record?</h4><p>A track record preserves earlier rankings and measures what happened afterward. <span class="learning-term">Hit rate</span> is the percentage of evaluated selections that beat their peer group. A short history can be noisy, so sample size matters.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>What is a backtest?</h4><p>A backtest asks: “What would this system have selected using only information available at that past date?” It can reveal strengths and weaknesses, but it cannot perfectly reproduce real investing and may be affected by the small company universe and survivorship bias.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>How to read the likelihood</h4><p>A 60% research likelihood means similar historical observations outperformed peers about six times out of ten after the model’s conservative adjustment. It does not mean the stock has a 60% chance of rising, and it is not a guarantee.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>How to read operating scenarios</h4><p>The conservative, base, and optimistic paths show different possible business outcomes. They project revenue and free cash flow—not the future stock price. The purpose is to understand what assumptions would need to be true under different conditions.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>Capital-efficiency basics</h4><p><span class="learning-term">CapEx</span> is spending on long-lived assets such as equipment and data centers. <span class="learning-term">R&amp;D</span> is research and product-development expense. <span class="learning-term">Free cash flow</span> is operating cash flow minus cash capital expenditures. Strong efficiency generally means a company can fund growth without excessive financial strain.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>Valuation basics</h4><p>Valuation asks how much investors currently pay for each dollar of sales, earnings, or free cash flow. A lower multiple is not automatically better, because faster-growing or stronger companies may deserve a premium. Phase 8 compares valuation with both peers and growth quality.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>How to read filing reactions</h4><p>An SEC filing may coincide with a market move, but the filing may not be the only cause. The one-day and five-day figures are observations around the filing date, not proof of causation and not forecasts of the next move.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>What is integrated evidence?</h4><p>This section gathers the major research layers into one summary. Supporting evidence shows favorable measurements; counter-evidence shows weaknesses or uncertainty. A favorable evidence balance still does not remove risk.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>How to use daily alerts</h4><p>Alerts tell you what changed enough to deserve a new review. “High priority” means the change was larger or involved several measurements; it does not mean “buy” or “sell.”</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>What is a subsector?</h4><p>A subsector groups companies with more similar business roles, such as AI platforms, semiconductors, or AI software. Comparing within a subsector is usually fairer than comparing a small software company directly with a giant cloud platform.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>How to read the market-direction chart</h4><p>Each line is normalized to 100 at the beginning of the selected period. A line at 125 means a 25% gain from that starting point. Equal weighting gives each available AI company the same influence, regardless of company size.</p></div>
  ...
  <div class="learning-mode-only learning-box"><h4>How to read the company ranking</h4><p>The original score summarizes growth, cash flow, financial strength, profitability, momentum, and strategic investment. Use the detailed profile to see the components and data coverage rather than relying only on the final number.</p></div>
...
function saveLearningMode(){localStorage.setItem('siteLearningMode',document.body.classList.contains('learning-mode')?'on':'off');}
function applyLearningMode(){const on=localStorage.getItem('siteLearningMode')==='on';document.body.classList.toggle('learning-mode',on);const btn=document.getElementById('learning-mode-toggle');if(btn){btn.textContent=on?'🎓 Learning Mode: On':'🎓 Learning Mode: Off';btn.setAttribute('aria-pressed',on?'true':'false');}}
document.getElementById('learning-mode-toggle').addEventListener('click',function(){document.body.classList.toggle('learning-mode');saveLearningMode();applyLearningMode();});applyLearningMode();
...
def main() -> None:
    ...
    output = {
        "schema_version": 12,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "live Phase 13 learning-mode and subsector research prototype" if any(c.get("score") is not None for c in companies) else "partial / cached prototype",
