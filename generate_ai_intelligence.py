...
<div class="container">
  ...
  <div class="title-row"><h1>AI Market Intelligence</h1><span class="premium">PREMIUM PREVIEW</span></div>
  <p class="timestamp" id="updated-at">Loading the latest AI company dataset...</p>
  <div class="notice"><strong>Research framework—not investment advice.</strong> The company score organizes reported financial facts and market momentum using the published methodology below. It is not yet a probability forecast, price target, or buy/sell recommendation. SEC figures can differ across issuers because companies use different permitted XBRL tags and fiscal calendars.</div>
  ...
  <p class="section-note">Click a company to open its reported financial history, score components, automated factual summary, and recent SEC filings.</p>
...
def main() -> None:
    ...
    output = {
        "schema_version": 1,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "live prototype" if any(c.get("score") is not None for c in companies) else "partial / cached prototype",
