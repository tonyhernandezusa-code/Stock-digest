    ...
    <div class="calc">
      ...
      <div id="financing-form" style="display:none;">
        ...
        <div id="arm-fields" style="display:none;">
          <label>Fixed-Rate Period (years) - the rate above holds until this point</label>
          <input type="number" step="0.5" id="d-armfixed" placeholder="e.g. 5" style="max-width:160px;">
          <label>Assumed Rate Increase Per Year After (percentage points) - for stress-testing, e.g. 0.5 means the rate goes up 0.5% every year once the fixed period ends</label>
...
function simulateLoanAmortization(loanAmount, initialRatePct, amortTermYears, interestOnlyYears, isARM, armFixedYears, armRateIncreasePerYear, numYears) {
  ...
  for (var month = 0; month < numYears * 12; month++) {
    var newRate = initialRatePct;
    if (isARM && month >= armFixedMonths) {
      var yearsPastFixed = Math.floor((month - armFixedMonths) / 12);
      newRate = initialRatePct + (yearsPastFixed + 1) * (armRateIncreasePerYear || 0);
    }
...
function saveFinancingDetails() {
  ...
  var updates = {
    ...
    armFixedYears: parseFloat(document.getElementById("d-armfixed").value) || 0,
...
function generate5And10YearProFormaPDF() {
  ...
  var simulatedYears = loanAmount > 0
    ? simulateLoanAmortization(loanAmount, interestRate, loanTermYears, interestOnlyYears, isARM, armFixedYears, armRateIncreasePerYear, startYearOffset + 10)
    : [];
