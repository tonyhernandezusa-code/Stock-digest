...
function rewindForD3(geojson) {   function fixPolygonRings(rings) {     var test = { type: "Feature", properties: {}, geometry: { type: "Polygon", coordinates: rings } };     if (d3.geoArea(test) > 2 * Math.PI) {       rings.forEach(function(ring) { ring.reverse(); });     }   }   var features = geojson.features.map(function(f) {     var geom = JSON.parse(JSON.stringify(f.geometry));     if (geom.type === "Polygon") {       fixPolygonRings(geom.coordinates);     } else if (geom.type === "MultiPolygon") {       geom.coordinates.forEach(fixPolygonRings);     }     return { type: "Feature", properties: f.properties, geometry: geom };   });   return { type: "FeatureCollection", features: features }; }
