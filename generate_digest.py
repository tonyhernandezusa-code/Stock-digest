...
function addUnitBatch() {
  ...
  // Number placeholders starting after any existing units of the same type, so adding
  // more of the same type later doesn't collide with earlier numbering.
  var existingOfType = currentUnits.filter(function(u) { return u.unitType === unitType; }).length;
  ...
  for (var i = 1; i <= count; i++) {
    ...
    batch.set(docRef, {
      type: "unit",
      unitType: unitType,
      label: unitType + " - " + (existingOfType + i),
      rent: rent,
      occupied: true,
      createdAt: firebase.firestore.FieldValue.serverTimestamp()
    });
...
function renameUnit(unitId, newLabel) {
  if (!newLabel.trim()) return;
  db.collection("properties").doc(currentPropertyId).collection("entries").doc(unitId).update({ label: newLabel.trim() });
}

function toggleUnitOccupied(unitId, occupied) {
  db.collection("properties").doc(currentPropertyId).collection("entries").doc(unitId).update({ occupied: occupied });
}

function renderUnits() {
  var el = document.getElementById("unit-list");
  if (!currentUnits.length) { el.innerHTML = "<p class='note'>No units yet.</p>"; return; }
  var sorted = currentUnits.slice().sort(function(a, b) { return (a.label || "").localeCompare(b.label || ""); });
  var html = "<div class='table-wrap'><table><tr><th>Type</th><th>Unit #</th><th style='text-align:right;'>Rent</th><th>Occupied</th><th></th></tr>";
  sorted.forEach(function(u) {
    html += "<tr><td>" + (u.unitType || "N/A") + "</td>" +
      "<td><input type='text' value='" + (u.label || "").replace(/'/g, "&#39;") + "' data-rename-id='" + u.id + "' style='width:120px;padding:4px;font-size:12px;'></td>" +
      "<td style='text-align:right;'>$" + Number(u.rent).toLocaleString() + "</td>" +
  ...
  html += "</table></div>";
  el.innerHTML = html;
  el.querySelectorAll("input[data-rename-id]").forEach(function(input) {
    input.addEventListener("blur", function() {
      renameUnit(input.getAttribute("data-rename-id"), input.value);
    });
  });
