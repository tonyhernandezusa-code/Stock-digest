...
function renderSummary() {
  ...
  var effectiveThisMonth = getEffectiveMonthlyExpenses(currentExpenses, thisYear, thisMonthIdx);
  var totalThisMonthExpenses = effectiveThisMonth.reduce(function(sum, e) { return sum + e.amount; }, 0);
  ...
  document.getElementById("detail-summary").innerHTML =
    ...
    "Expenses this month: $" + totalThisMonthExpenses.toLocaleString(undefined, {maximumFractionDigits: 0}) + " &middot; All-time logged expenses: $" + totalAllExpenses.toLocaleString() + "<br>" +
    ...
    "<br><span style='font-size:11px;color:#888;'>Cash flow is rent from occupied units minus this month's expenses. Expenses flagged as Annual (property tax, insurance, etc.) are divided by 12 and spread evenly across all 12 months rather than counted entirely in the month they were logged. A simple estimate, not a full P&amp;L.</span>";
}
...
function generateYTDReportPDF() {
  ...
  for (var m = 0; m <= currentMonthIdx; m++) {
    var effectiveExpenses = getEffectiveMonthlyExpenses(currentExpenses, year, m);
    var variableTotal = effectiveExpenses.reduce(function(sum, e) { return sum + e.amount; }, 0);
    var totalExpenses = fixedMonthlyCosts + variableTotal;
    var netCashFlow = totalMonthlyRent - totalExpenses;

    ytdIncome += totalMonthlyRent;
    ytdFixed += fixedMonthlyCosts;
  ...
  rows.push([
    "YTD Total", "$" + ytdIncome.toLocaleString(), "$" + ytdFixed.toLocaleString(),
...
// Given all logged expenses, returns the "effective" expenses for one specific month:
// non-annual expenses dated exactly in that month count in full; expenses flagged "annual"
// (property tax, insurance, etc.) are divided by 12 and counted in EVERY month of that year,
// rather than as one lump sum in whichever month they happened to be logged.
  ...
  allExpenses.forEach(function(e) {
    ...
    if (e.isAnnual) {
      if (d.slice(0, 4) === String(year)) {
        result.push({ category: e.category, amount: Number(e.amount || 0) / 12 });
      }
    } else if (d.slice(0, 7) === monthKey) {
      result.push({ category: e.category, amount: Number(e.amount || 0) });
    }
  });
  return result;
}
...
function generateIncomeStatementPDF() {
  ...
  // This month's effective expenses (annual-flagged ones already divided by 12), grouped by category
  var thisMonthEffective = getEffectiveMonthlyExpenses(currentExpenses, year, monthIdx);
  var thisMonthGroups = groupExpensesByCategory(thisMonthEffective);
  var thisMonthVariableTotal = thisMonthGroups.reduce(function(sum, g) { return sum + g.total; }, 0);

  // Year-to-date: accumulate each month's effective expenses (Jan through selected month) by category.
  // This correctly spreads an annual expense across every month regardless of which month it was actually logged in.
  var ytdEffective = [];
  for (var mm = 0; mm <= monthIdx; mm++) {
    ytdEffective = ytdEffective.concat(getEffectiveMonthlyExpenses(currentExpenses, year, mm));
  }
  var ytdGroups = groupExpensesByCategory(ytdEffective);
