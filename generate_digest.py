...
<div id="dashboard" style="display:none;">
  ...
  <div id="property-detail" style="display:none;">
    ...
    <div class="calc">
      ...
      <input type="text" id="u-label" placeholder="Apt 1A">
      <label>Monthly Rent</label>
      <input type="number" id="u-rent">
      <label><input type="checkbox" id="u-occupied" checked style="width:auto;display:inline-block;"> Currently occupied</label>
...
function addUnit() {
  ...
  var label = document.getElementById("u-label").value.trim();
  var rent = parseFloat(document.getElementById("u-rent").value) || 0;
  var occupied = document.getElementById("u-occupied").checked;
  showError("unit-error", "");
  if (!label) { showError("unit-error", "Enter a unit label."); return; }

  db.collection("properties").doc(currentPropertyId).collection("entries").add({
    type: "unit", label: label, rent: rent, occupied: occupied,
    createdAt: firebase.firestore.FieldValue.serverTimestamp()
  }).then(function() {
    document.getElementById("u-label").value = "";
    document.getElementById("u-rent").value = "";
    document.getElementById("u-occupied").checked = true;
...
function renderUnits() {
  ...
  var html = "<div class='table-wrap'><table><tr><th>Unit</th><th style='text-align:right;'>Rent</th><th>Status</th><th></th></tr>";
  currentUnits.forEach(function(u) {
    html += "<tr><td>" + u.label + "</td><td style='text-align:right;'>$" + Number(u.rent).toLocaleString() + "</td>" +
      "<td>" + (u.occupied ? "<span style='color:#1a8a3d;'>Occupied</span>" : "<span style='color:#c0392b;'>Vacant</span>") + "</td>" +
