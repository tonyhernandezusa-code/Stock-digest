...
function generateYTDReportPDF() {
  ...
  for (var m = 0; m <= currentMonthIdx; m++) {
    var monthKey = year + "-" + String(m + 1).padStart(2, "0");
    var monthExpenses = currentExpenses.filter(function(e) { return (e.date || "").slice(0, 7) === monthKey; });
    var variableTotal = monthExpenses.reduce(function(sum, e) { return sum + Number(e.amount || 0); }, 0);
  ...
  rows.push([
    ...
    "$" + ytdVariable.toLocaleString(), "$" + ytdTotalExpenses.toLocaleString(), "$" + ytdNetCashFlow.toLocaleString()
  ]);
