...
function generate5And10YearProFormaPDF() {
  ...
  doc.text("Generated: " + today + "  |  Rent Growth: " + (rentGrowth * 100).toFixed(1) + "%/yr  |  Expense Growth: " + (expenseGrowth * 100).toFixed(1) + "%/yr", 14, 32);

  doc.autoTable({
    startY: 40,
    head: [["Year", "Gross Income", "Op. Expenses", "NOI", "Debt Service", "Cash Flow", "Cumulative"]],
    body: rows,
    styles: { fontSize: 8 },
